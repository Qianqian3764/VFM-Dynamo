...
from .alt_cuda_corr.correlation import Correlation
from .net_blocks import FeaturePyramidExtractor, OpticalFlowEstimator, ContextNetwork


class PWCFlow(nn.Module):
    def __init__(self, lv_chs=(3, 16, 32, 64, 96, 128, 192), n_out=4, n_context=1,
                 search_range=7, bn=False):
    ...
    def _forward(self, x1_pyramid, x2_pyramid):
        ...
        for i, (x1, x2) in enumerate(zip(x1_pyramid, x2_pyramid)):
            ...
            if len(flows) == self.n_out:
                break
        flows = [F.interpolate(flow * 4, scale_factor=8,
                               mode='bilinear', align_corners=True) for flow in flows]
        return flows[::-1]

    def forward(self, im1,im2, with_bk=False):
        ...
        if with_bk:
            ...
            outputs[("flow", 0)] = flows_f[0]
            outputs[("flow", 1)] = flows_f[1]
            outputs[("flow", 2)] = flows_f[2]
            outputs[("flow", 3)] = flows_f[3]
            return outputs
            #return flows_f
