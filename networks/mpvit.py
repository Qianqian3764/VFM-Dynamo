...
#from mmseg.utils_uni import get_root_logger
    ...
    def __init__(self, Ch, h, window):
        ...
        window: Window size(s) in convolutional relative positional encoding.
                ...
                2. A dict mapping window size to #attention head splits
                   (e.g. {window size 1: #attention head splits 1, window size
                                      2: #attention head splits 2})
...
def mpvit_small(**kwargs):
    ...
    checkpoint = \
        torch.load(r'./mpvit_small.pth',
                   map_location=lambda storage, loc: storage)['model']
    logger = MMLogger.get_instance(name='MMLogger', logger_name='Logger')
    model_dict = model.state_dict()
    state_dict = {k: v for k, v in checkpoint.items() if k in model_dict.keys() and v.shape == model_dict[k].shape}
    model_dict.update(state_dict)
    model.load_state_dict(model_dict)
    del checkpoint
    del logger
    model.default_cfg = _cfg_mpvit()
    # model.num_ch_enc = np.array([64, 128, 216, 288, 288])
    return model
