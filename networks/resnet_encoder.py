...
class mvs_encoder(nn.Module):
    """Pytorch module for a resnet encoder
    """

    def __init__(self, num_layers, pretrained, num_input_images=1, **kwargs):
        super(mvs_encoder, self).__init__()
    ...
    def forward(self, input_image):
        ...
        x = self.encoder.conv1(x)  # //2
        x = self.encoder.bn1(x)
        self.features_mvs.append(self.encoder.relu(x))
        self.features_mvs.append(self.encoder.layer1(self.encoder.maxpool(self.features_mvs[-1])))  # //2

        return self.features_mvs[-1]
...
class FPN4(nn.Module):
    ...
    def forward(self, x):
        ...
        conv2 = self.conv2(conv1) #(6,32,48,160)
        ...
        out = self.out(intra_feat) #(6,32,48,160)
        if self.dcn:
