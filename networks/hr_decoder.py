...
class DepthDecoder_hr(nn.Module):
    ...
    def forward(self, input_features):
        outputs = {}
        feat={}
        feat[4] = self.convs["f4"](input_features[4]) #6,288 6,20
        feat[3] = self.convs["f3"](input_features[3])#6,288 12,40
        feat[2] = self.convs["f2"](input_features[2])#6,216 24,80
        feat[1] = self.convs["f1"](input_features[1])#6,128,48,160
        feat[0] = input_features[0]#6,64,96,320
        ...
        outputs[("d_feature", 1)] = features["X_04"]  # 32 96 320
        outputs[("d_feature", 2)] = features["X_13"]  # 64 48 160
        outputs[("d_feature", 3)] = features["X_22"]  # 128 24 80
        return outputs
