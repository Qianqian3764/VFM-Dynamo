...
def compute_rigid_flow(depth, pose, intrinsics, reverse_pose=False):
    ...
    # shape: (#batch,h*w,2)
    rigid_flow = tgt_coords - src_coords
    # rigid_flow = src_coords - tgt_coords
    # shape: (#batch,2,h,w)
    rigid_flow = rigid_flow.contiguous().view(batch_size, h, w, 2).permute(0, 3, 1, 2)
    return rigid_flow


def depth2flow1(depth, pose_mat, intrinsics, padding_mode='zeros'):
    """
    Depth + Pose => Flow
...
def inverse_warp2(img, depth, pose, intrinsics, ref_depth, rotation_mode='euler', padding_mode='zeros'):
    ...
    if isinstance(pose, list):
        for pp, p_vec in enumerate(pose):
            if pp == 0:
                ...
                aux_mat = torch.tensor([0, 0, 0, 1]).type_as(pose_mat).unsqueeze(0).unsqueeze(0).repeat(batch_size, 1,
                                                                                                        1)  # [B,1,1]
                pose_mat = torch.cat([pose_mat, aux_mat], dim=1)  # [B,4,4]
                continue;
            next_mat = pose_vec2mat(p_vec, rotation_mode)  # RT, [B,3,4]
            aux_mat = torch.tensor([0, 0, 0, 1]).type_as(next_mat).unsqueeze(0).unsqueeze(0).repeat(batch_size, 1,
                                                                                                    1)  # [B,1,1]
    ...
    if np.array(torch.__version__[:3]).astype(float) >= 1.3:
        projected_depth = F.grid_sample(ref_depth, src_pixel_coords, padding_mode=padding_mode,
                                        align_corners=True).clamp(min=1e-3)
    else:
